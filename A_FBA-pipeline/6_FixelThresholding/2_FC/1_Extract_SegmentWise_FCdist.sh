...
# Write out the distribution of fc for each fixel within each ROI segment per tract for each subject
cd "$tract_dir" || exit
for tract in *; do 
    cd "$tract_dir/$tract" || continue
    folder="fixel_masks_fc"
    cd "$folder" || continue
    for segment in "${segments[@]}"; do
        subfolder="${tract}_${segment}"
        if [ -d "$subfolder" ]; then
            cd "$subfolder" || continue
            for masks in fc_${tract}_${segment}; do
                if [ -d "$masks" ]; then
                    cd "$masks" || continue
                    for subject in 10*; do
                        echo "$tract $folder $subfolder $masks $subject"
                        # Uncomment the following line if you need to run mrstats
                        #mrstats "$subject" > "${subject}_${subfolder}_meanfc.txt" -force
                        mrdump "$subject" > "${subject}_${subfolder}_fc_distribution.txt" -force
                    done
                    cd ..
                fi
            done
