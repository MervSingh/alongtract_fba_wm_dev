...
if 'MeanFD' not in df.columns:
    raise ValueError("MeanFD column not found. This script is FD-only.")

df = df[['TractName', 'Hemi', 'Segment', 'MeanFD']].copy()
df = df[df['MeanFD'].notna()]

df = df.rename(columns={'MeanFD': 'MeanValue'})
...
output_csv = os.path.join(base_dir, 'MeanFD_SEM_per_segment.csv')
output_png = os.path.join(base_dir, 'ALL_Tracts_FD_SEM.png')
...
out = out[
    ['TractName', 'Hemi', 'Segment', 'MeanFD', 'StdFD', 'SEM', 'count']
...
def format_ax(ax):
    ...
    ax.set_ylabel("Mean FD", fontsize=8)
...
fig.suptitle(
    'Along-Tract Mean FD ± SEM Profiles',
    fontsize=18,
    y=0.995
)
