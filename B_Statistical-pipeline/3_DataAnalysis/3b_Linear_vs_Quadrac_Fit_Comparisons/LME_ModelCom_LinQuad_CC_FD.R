...
for (tract in unique(df$TractName)) {
  ...
  for (seg in unique(tract_data$Segment)) {
    ...
    seg_data <- seg_data %>%
      ...
      # =====================================================
      model_quadred <- lmer(
        MeanFD ~ AgeAtScan + I(AgeAtScan^2) + Gender + (1 | ID),
        data = seg_data,
        REML = FALSE
      )
      ...
      # =====================================================
      model_summaries[[paste(tract, seg, "linred", sep = "_")]] <-
        broom.mixed::tidy(model_linred, effects = "fixed") %>%
        mutate(Model = "Linear",
               Tract = tract,
               Segment = seg)
      
      model_summaries[[paste(tract, seg, "quadred", sep = "_")]] <-
...
write_csv(
  model_summaries_df,
  "./Lin_Quad_Reduced/CC_ModelSummaries_LinRed_QuadRed_FD.csv"
)

write_csv(
  delta_df,
  "./Lin_Quad_Reduced/LinQuad_DeltaAIC_CC_FD.csv"
)
...
p_heat <- ggplot(delta_df,
  ...
  labs(
    title = "Comparisons between Linear and Quadratic (MeanFD models)",
    subtitle = expression(Delta*"AIC = Linear Model - Quadratic Model"),
    x = "Segment",
...
# =========================================================
ggsave(
  "./Lin_Quad_Reduced/LinQuad_DeltaAIC_Heatmap_CC_FD.pdf",
  p_heat,
  width = 12,
  height = 5,
  device = cairo_pdf
...
pdf(
  "./Lin_Quad_Reduced/AgeAtScan_ForestPlots_LinRed_vs_QuadRed_CC_FD_TDS.pdf",
  width = 12,
  height = 5,
  useDingbats = FALSE
)
...
for (tract in unique(model_summaries_df$Tract)) {
  ...
  forest_plot <- ggplot(
    ...
    # Significance stars (Linear only)
    geom_text(
...
plot_lme_marginal_linquad_reduced_fc <- function(
 ...
 model_quad <- lmer(
    MeanFD ~ AgeAtScan + I(AgeAtScan^2) +
      Gender +
      (1 | ID),
    data = seg_data,
    REML = FALSE
  ...
  X_quad <- model.matrix(
    ~ AgeAtScan + I(AgeAtScan^2) + Gender,
    newdata
  )
...
pdf(
  "./Lin_Quad_Reduced/MarginalPlots_LinQuad_ReducedModel_CC_FD_v2.pdf",
  width = 11,
  height = 8.5,
  useDingbats = FALSE
)
...
cat("DONE: marginal reduced-model linear-quadratic FD plots saved.\n")
...
pdf(
  "./Lin_Quad_Reduced/MarginalPlots_LinQuad_ReducedModel_CC_FD_4x2.pdf",
  width = 12,
  height = 20,
  useDingbats = FALSE
)
