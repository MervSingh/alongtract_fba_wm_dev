...
df <- read_csv("/PATH/TO/INPUT/AssocProj_ModelSummaries_Reduced_Full_TDS_FD.csv",
               col_types = cols(
                 .default = col_guess(),
                 p.adjusted = col_double(),
                 sig = col_character()
               ))
df = df %>% filter(Model == "Reduced")
...
write_csv(
  combined_segmented,
  "/PATH/TO/OUTPUT/AssocProj_Relabeled_FD.csv"
)
...
fit_group_models_raw <- function(data, group_tracts, group_label) {
  ...
  formula <- if (group_label == "AP_DS") {
    ...
    # Projection tracts: only segment
...
write_csv(
  all_std,
  "/PATH/TO/OUTPUT/AssocProj_GradModels_Results_FD.csv"
)
