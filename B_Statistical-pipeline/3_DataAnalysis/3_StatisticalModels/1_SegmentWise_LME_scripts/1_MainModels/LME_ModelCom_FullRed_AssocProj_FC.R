...
write_csv(
  model_summaries_df,
  "./LME/output/AssocProj_ModelSummaries_Reduced_Full_TDS_FC.csv"
)
...
# =========================================================
write_csv(delta_df,
          "./LME/output/DeltaAIC_TDS_AssocProj_FC.csv")
...
# =========================================================
ggsave(
  "./LME_Plots/DeltaAIC_TDS_Heatmap_AssocProj_FC.pdf",
  p_heat,
  width = 14,
  height = 6,
  device = cairo_pdf
...
pdf(
  "./LME_Plots/AgeAtScan_ForestPlots_Reduced_vs_Full_AssocProj_FC_TDS.pdf",
  width = 18,
  height = 6,
  useDingbats = FALSE
)
...
plot_lme_marginal_reduced <- function(df,
                                      tract,
                                      hemi,
                                      seg,
                                      show_spaghetti = TRUE) {
  ...
  seg_data <- df %>%
    ...
    mutate(
      ...
      # Match your model pipeline
      AgeAtScan = AgeAtScan - mean(AgeAtScan, na.rm = TRUE),
      
      Gender = factor(Gender),
      
      IntraCranialVol =
        scale(IntraCranialVol,
              center = TRUE,
              scale = FALSE)[,1]
    )
...
pdf(
  "./LME_Plots/MarginalPlots_ReducedModel_AssocProj_FC_v2.pdf",
  width = 11,
  height = 8.5,
  useDingbats = FALSE
)
...
cat("DONE: marginal reduced-model plots saved.\n")