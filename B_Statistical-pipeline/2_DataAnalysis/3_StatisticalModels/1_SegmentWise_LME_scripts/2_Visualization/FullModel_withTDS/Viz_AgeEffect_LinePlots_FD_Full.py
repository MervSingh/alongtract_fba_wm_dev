...
# ---- Load model results ----
df_cc = pd.read_csv('/PATH/TO/INPUT/AssocProj_ModelSummaries_Reduced_Full_TDS_FD.csv')
df_other = pd.read_csv('/PATH/TO/INPUT/CC_ModelSummaries_Reduced_Full_TDS_FD.csv')

# Combine in memory
df = pd.concat([df_cc, df_other], ignore_index=True)

# keep only Full model results
df = df[df['Model'] == 'Full']
...
# ---- Plotting ----
output_pdf = '/PATH/TO/OUTPUT/AgeAtScan_FD_lineplots_Full_v2.pdf'
...
with PdfPages(output_pdf) as pdf:
    ...
    if cc_tracts:
        ...
        ax.set_ylabel("Age change in Mean FD")
